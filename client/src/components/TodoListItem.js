...
const TodoListItem = ({ todo }) => {
  ...
  const remove = () => deleteTodo({ variables: { id: todo.id } })

  const buttonText = todo.completed ? 'mark as to be done' : 'mark as done'

  return (
    <li>
      {todo.text} <button onClick={update}>{buttonText}</button>{' '}
